...
if __name__ == '__main__':
    ...
    parser.add_argument('-trec_file', default="step_360000.trec")
    parser.add_argument('-top1_file', default="top1_click")

    args = parser.parse_args()

    trec_file = open(args.trec_file)
    top1_file = open(args.top1_file)

    # read trec
    trec = {}
    all_doc = set()
    for line in trec_file:
        #  1 Q0 sogou-15630 1 0.601785
        items = line.split()
        qid = int(items[0])
        docid = items[2].strip()
        rank = int(items[3])
