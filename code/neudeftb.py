...
class neudeftb(nn.Module):
    def __init__(self, opt):
        super(neudeftb, self).__init__()
    ...
    def forward(self, inputs_q, inputs_d, inputs_dcq, inputs_dbody, masks_q, masks_d, masks_dcq, masks_dbody):
        ...
        split_embed_dcq = concat_embed_dcq.view(bs, max_num, max_len, -1)
        ...
        for i in range(max_num):
            ...
            qcqdattn[i] = F.tanh(self.dcqdense(self.knrm_logsum(embed_d, i_dcq, masks_d, m_dcq))) # bs * 1
            qcqbattn[i] = F.tanh(self.bcqdense(self.knrm_logsum(embed_b, i_dcq, masks_dbody, m_dcq)))  # bs * 1
